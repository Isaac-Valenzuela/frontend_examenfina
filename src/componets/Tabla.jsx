...
const Tabla = () => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (confirmar) {
                const token = localStorage.getItem('token')
                const url = `http://localhost:5000/api/paciente/eliminar/${id}`
                const headers = {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                }
        ...
        <>
            {
                pacientes.length == 0
                    ...
                    <table className='w-full mt-5 table-auto shadow-lg  bg-white'>
                        <thead className='bg-red-700 text-slate-400'>
                            <tr>
                        ...
                        <tbody>
                            {
                                pacientes.map((paciente, index) => (
                                    <tr className="border-b hover:bg-gray-300 text-center" key={paciente._id}>
                                        ...
                                        <td>
                                            <span className="bg-blue-100 text-pink-500 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{paciente.estado && "activo"}</span>
                                        </td>
